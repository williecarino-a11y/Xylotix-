...
const loginLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => res.status(429).json(limiterMessage('AUTH_RATE_LIMITED', 'Too many login attempts. Please wait a few minutes and try again.'))
});
...
const registerLimiter = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  handler: (req, res) => res.status(429).json(limiterMessage('REGISTRATION_RATE_LIMITED', 'Too many account creation attempts. Please try again later.'))
});
...
const verificationLimiter = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  handler: (req, res) => res.status(429).json(limiterMessage('VERIFICATION_RATE_LIMITED', 'Too many verification attempts. Please wait a few minutes and try again.'))
});
...
const codeRequestLimiter = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  handler: (req, res) => res.status(429).json(limiterMessage('CODE_REQUEST_RATE_LIMITED', 'Too many requests. Please wait a few minutes and try again.'))
});
...
const resetPasswordLimiter = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  handler: (req, res) => res.status(429).json(limiterMessage('PASSWORD_RESET_RATE_LIMITED', 'Too many password reset attempts. Please wait a few minutes and try again.'))
});
...
async function createSession(userId) {
  ...
  await UserSession.create({ userId, tokenHash: hashToken(token), expiresAt });
  return token;
}
...
router.post('/register', registerLimiter, async (req, res) => {
  try {
    ...
    if (Number.isNaN(parsedDate.getTime()) || parsedDate.toISOString().slice(0, 10) !== dateOfBirth) return res.status(400).json({ status: 'error', code: 'INVALID_DATE_OF_BIRTH', message: 'Enter a valid date of birth.' });
    ...
    if (!passwordResult.valid) return res.status(400).json({ status: 'error', code: passwordResult.errors[0].code, message: passwordResult.errors[0].message });
    ...
    let user;
    try {
      user = await User.create({ firstName, lastName, email, gender, dateOfBirth: parsedDate, passwordHash, emailVerified: false, accountVerified: false });
    } catch (error) {
      if (error && error.code === 11000) return res.status(409).json({ status: 'error', code: 'ACCOUNT_EXISTS', message: 'An account with that email already exists.' });
      throw error;
    }
    ...
    return res.status(201).json({ status: 'success', data: { verificationRequired: true, verificationMethod: 'email', maskedEmail: maskEmail(user.email), expiresInSeconds: VERIFICATION_MINUTES * 60 } });
  } catch (error) {
...
router.post('/reset-password', resetPasswordLimiter, async (req, res) => {
  try {
    ...
    const reset = await PasswordResetToken.findOne({ tokenHash, expiresAt: { $gt: new Date() }, usedAt: null });
    ...
    if (!claimed) return res.status(400).json({ status: 'error', code: 'INVALID_RESET_TOKEN', message: 'This password reset link is invalid or has expired.' });

    const updatedUser = await User.findByIdAndUpdate(reset.userId, { passwordHash }, { new: true });
    if (!updatedUser) throw new Error('Password reset target user was not found.');
