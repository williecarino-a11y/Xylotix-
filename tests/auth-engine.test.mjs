...
test('configuration contains the complete registration flow', () => {
  assert.deepEqual(Object.keys(AUTH_FLOW.register.steps), ['welcome', 'name', 'email', 'birthday', 'password', 'verification', 'authenticated']);
});

test('empty birthday is required and does not become an internal backend error', () => {
  const errors = validation.validateStep(AUTH_FLOW.register.steps.birthday, { gender: 'male', dateOfBirth: '' });
...
test('invalid birthday is rejected while a valid adult birthday passes', () => {
  ...
  assert.equal(validation.validateStep(step, { gender: 'female', dateOfBirth: '2026-99-99' }).dateOfBirth.code, 'authBirthdayInvalid');
  assert.equal(validation.validateStep(step, { gender: 'female', dateOfBirth: '1990-01-01' }).dateOfBirth, undefined);
});

test('confirmation validation follows the source password', () => {
  const step = AUTH_FLOW.register.steps.password;
  assert.equal(validation.validateStep(step, { password: 'correct123', confirmPassword: 'different123' }).confirmPassword.code, 'authPasswordMismatch');
  assert.equal(validation.validateStep(step, { password: 'correct123', confirmPassword: 'correct123' }).confirmPassword, undefined);
});
