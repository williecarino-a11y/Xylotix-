...
export class AuthRenderer {
  ...
  ensureViewportStyles() {
    ...
    style.textContent = `
      html.miimiid-auth-active,body.miimiid-auth-active{margin:0!important;padding:0!important;min-height:100%;overflow-x:hidden}
  ...
  claimViewport() {
    ...
    const ids = ['miimiid-app-shell','miimiid-fun-center-view','miimiid-ai-tutor-view','course-list-view','course-detail-view','lesson-view'];
    ids.forEach(id => document.getElementById(id)?.classList.add('hidden'));
    document.documentElement.classList.add('miimiid-auth-active');
    document.body.classList.add('miimiid-auth-active');
    return root;
  }

  bind() {
    if (this.bound) return;
    const root = this.claimViewport();
    root.addEventListener('input', event => { const field=event.target.closest('[data-auth-field]'); if(field)this.controller.fieldChanged(field.dataset.authField,field.value); });
    root.addEventListener('change', event => { const field=event.target.closest('[data-auth-field]'); if(field)this.controller.fieldChanged(field.dataset.authField,field.value); });
    root.addEventListener('focusin', event => { const field=event.target.closest('[data-auth-field]'); if(field)this.controller.fieldFocused(field.dataset.authField); });
    root.addEventListener('focusout', event => { const field=event.target.closest('[data-auth-field]'); if(field)this.controller.fieldBlurred(field.dataset.authField); });
    root.addEventListener('click', event => {
      const action=event.target.closest('[data-auth-action]');
      if(!action)return;
      event.preventDefault();
      const id=action.dataset.authAction;
      if(id==='switch-login')return this.controller.setFlow('login');
      if(id==='switch-register')return this.controller.setFlow('register');
      if(id==='switch-forgot')return this.controller.setFlow('forgot');
      if(id==='back')return this.controller.back();
      if(id==='primary')return this.controller.primaryAction();
      return this.controller.secondaryAction(id);
    });
    this.bound=true;
  }
  ...
  renderField(field,state){
    ...
    if(field.type==='select'){
      const options=(field.options||[]).map(option=>`<option value="${escapeHtml(option)}" ${String(value)===option?'selected':''}>${escapeHtml(option==='male'?t('authMale','Male'):t('authFemale','Female'))}</option>`).join('');
  ...
  renderLinks(state){
    ...
    if(state.flow==='register'&&state.step!=='welcome'&&state.step!=='verification')links.unshift(`<button type="button" class="miimiid-auth-link miimiid-auth-back" data-auth-action="back">${escapeHtml(t('authBack','Back'))}</button>`);
  ...
  render(){
    ...
    let content;
    if(isConfirmation){
      const message=state.flow==='forgot'?t('authResetInstructionsSent','Reset instructions have been sent.'):state.flow==='reset'?t('authPasswordResetSuccess','Your password has been reset successfully.'):t('authAccountCreated','Your account is ready.');
      content=`<p class="miimiid-auth-welcome">${escapeHtml(message)}</p>`;
    }else if(isWelcome){content=`<p class="miimiid-auth-welcome">${escapeHtml(subtitle)}</p>`;}else{content=fields;}
