...
export class AuthErrorNormalizer {
  normalize(error, response = null) {
    ...
    if (!response) return new AuthError({ type: 'network', code: 'NETWORK_ERROR', message: 'Unable to connect to the authentication service.', retryable: true, source: 'network' });
    const body = error || {};
...
export class AuthService {
  ...
  async request(endpoint, payload, method = 'POST') {
    ...
    try {
      ...
      throw this.normalizer.normalize(error);
    }

    let result = null;
    try { result = await response.json(); }
    catch (_) {
      throw new AuthError({ type: 'server', code: 'INVALID_RESPONSE', message: 'The authentication service returned an invalid response.', source: 'server' });
    }

    if (!response.ok || result?.status !== 'success') {
      throw this.normalizer.normalize({
        code: result?.code || result?.error?.code,
        field: result?.field || result?.error?.field,
  ...
  resend(v) { return this.request('/api/auth/resend-verification', { email: v.email }); }
  resendVerification(v) { return this.resend(v); }
  forgot(v) { return this.request('/api/auth/forgot-password', { email: v.email }); }
  forgotPassword(v) { return this.forgot(v); }
  reset(v) {
    const token = new URLSearchParams(window.location.search).get('resetToken');
    return this.request('/api/auth/reset-password', { token, password: v.password });
  }
  resetPassword(v) { return this.reset(v); }
...
export class SessionManager {
  ...
  async restore() {
    ...
    try {
      ...
      this.user = result?.data?.user || result?.user || null;
