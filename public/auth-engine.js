...
(function () {
  ...
  const LABELS = {
    ...
    confirmPassword: 'Confirm password', gender: 'Gender', dateOfBirth: 'Birthday', code: 'Verification code',
    male: 'Male', female: 'Female', authContinue: 'Continue', authGetStarted: 'Get started',
    authSignIn: 'Sign in', authCreateAccountButton: 'Create account', authVerifyAccount: 'Verify',
    authResendCode: 'Resend code', authSendResetInstructions: 'Send reset instructions',
  ...
  function fieldMarkup(field) {
    ...
    if (field.type === 'select') {
      input = `<select id="${field.domId}" name="${field.id}"${required}><option value="">Select ${label.toLowerCase()}</option>${(field.options || []).map(v => `<option value="${v}">${text(v)}</option>`).join('')}</select>`;
    } else {
      const type = field.type === 'birthday' ? 'date' : (field.type === 'verification' ? 'text' : field.type);
      const inputMode = field.type === 'verification' ? ' inputmode="numeric" maxlength="6" autocomplete="one-time-code"' : '';
      input = `<input id="${field.domId}" name="${field.id}" type="${type}" autocomplete="${field.type === 'password' ? 'new-password' : field.type}"${inputMode}${required}>`;
    }
    return `<div class="miimiid-auth-field"><label for="${field.domId}">${label}${field.required ? ' *' : ''}</label>${input}</div>`;
  }

  function actionMarkup(action, secondary = false) {
    return `<button type="button" id="${action.domId}" class="miimiid-auth-action${secondary ? ' secondary' : ''}">${text(action.label)}</button>`;
  }

  function createAuthShell() {
    if (document.getElementById('miimiid-auth-view')) return;

    const appChildren = Array.from(document.body.children);
    const appShell = document.createElement('div');
    appShell.id = 'miimiid-app-shell';
    appShell.className = 'miimiid-app-shell hidden';
    appShell.dataset.authEngineAppShell = '1';
    for (const child of appChildren) appShell.appendChild(child);
    ...
    auth.innerHTML = `
      <div class="miimiid-auth-card">
        <div class="miimiid-auth-header"><strong>Miimiid</strong><div id="miimiid-auth-progress"></div></div>
        <div id="miimiid-auth-status" class="miimiid-auth-status hidden" role="alert" aria-live="polite"></div>
        <div id="miimiid-auth-content"></div>
      </div>`;
    ...
    Object.entries(AUTH_FLOW).forEach(([flowName, flow]) => {
      ...
      Object.entries(flow.steps).forEach(([stepId, step]) => {
        ...
        if (flowName === 'register') section.dataset.registerStep = stepId;
    ...
    document.head.insertAdjacentHTML('beforeend', `<style id="miimiid-auth-shell-style">
      .miimiid-auth-shell{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;box-sizing:border-box;background:#0f172a;color:#f8fafc}.miimiid-auth-card{width:min(100%,460px);background:#1e293b;border:1px solid #334155;border-radius:24px;padding:24px;box-shadow:0 24px 70px rgba(0,0,0,.35)}.miimiid-auth-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;font-size:1.25rem}.miimiid-auth-step h1{font-size:1.6rem;margin:0 0 20px}.miimiid-auth-field{display:grid;gap:7px;margin-bottom:16px}.miimiid-auth-field label{font-size:.9rem;font-weight:600}.miimiid-auth-field input,.miimiid-auth-field select{width:100%;min-height:50px;padding:13px 14px;border-radius:12px;border:1px solid #475569;background:#0f172a;color:#f8fafc;font-size:16px;box-sizing:border-box}.miimiid-auth-field input:focus,.miimiid-auth-field select:focus{outline:2px solid #38bdf8;outline-offset:1px}.miimiid-auth-field-error{margin-top:-9px;margin-bottom:12px;color:#f87171;font-size:.82rem}.miimiid-auth-actions{display:grid;gap:10px;margin-top:20px}.miimiid-auth-action{position:relative;width:100%;min-height:50px;border:0;border-radius:12px;background:#0284c7;color:white;font-weight:700;font-size:16px;cursor:pointer}.miimiid-auth-action.secondary,.miimiid-auth-mode{background:transparent;color:#7dd3fc;border:1px solid #334155}.miimiid-auth-action:disabled{opacity:.55;cursor:not-allowed}.miimiid-auth-modes{display:flex;justify-content:center;gap:8px;flex-wrap:wrap;margin-top:18px}.miimiid-auth-mode{padding:9px 12px;border-radius:10px;cursor:pointer}.miimiid-auth-status{padding:12px;border-radius:10px;background:#450a0a;color:#fecaca;margin-bottom:16px}.miimiid-auth-shell .miimiid-auth-step.hidden{display:none!important}@media(max-width:520px){.miimiid-auth-shell{padding:16px}.miimiid-auth-card{padding:20px;border-radius:20px}}
    </style>`);
  }
  ...
  function boot() {
    ...
    if (!resetToken) controller.restoreSession().then(user => { if (user) showAuthenticated(user); });
  }
