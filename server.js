...
`
    ...
    html = html.replace(
      /if\s*\(\s*document\.readyState\s*===\s*["']loading["']\s*\)\s*\{\s*document\.addEventListener\(\s*["']DOMContentLoaded["']\s*,\s*initializeMiimiidApplication\s*\)\s*;\s*\}\s*else\s*\{\s*initializeMiimiidApplication\(\s*\)\s*;\s*\}/,
      ''
    );
